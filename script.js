...
function setCloudBusy(isBusy) {
  ...
  if (els.loginCloudButton) {
    els.loginCloudButton.disabled = isBusy || getCloudLoginWaitSeconds() > 0;
  }
}

function getCloudLoginWaitSeconds() {
  const nextAt = Number(localStorage.getItem(CLOUD_LOGIN_NEXT_AT_KEY) || 0);
  return Math.max(0, Math.ceil((nextAt - Date.now()) / 1000));
}

function setCloudLoginCooldown(durationMs = CLOUD_LOGIN_COOLDOWN_MS) {
  localStorage.setItem(CLOUD_LOGIN_NEXT_AT_KEY, String(Date.now() + durationMs));
}

function clearCloudLoginCooldown() {
  localStorage.removeItem(CLOUD_LOGIN_NEXT_AT_KEY);
}

function isSupabaseRateLimit(error) {
...
function getReadableError(error) {
  ...
  try {
    ...
    return text && text !== "{}" ? text : "Supabase не вернул текст ошибки";
  } catch {
    return String(error);
  }
}
...
async function manualCloudSync() {
  ...
  try {
    ...
    if (cloudConfig.status === "synced") {
      alert("Синхронизация готова. Книги и слова сохранены в облаке.");
    } else if (cloudConfig.lastError) {
      alert(`Синхронизация не прошла.\n\nОшибка: ${cloudConfig.lastError}`);
    }
  } catch (error) {
    cloudConfig.status = "offline";
    cloudConfig.lastError = error?.message || "Manual sync failed";
    saveCloudConfig();
    renderSyncStatus();
    alert(`Синхронизация не прошла.\n\nОшибка: ${cloudConfig.lastError}`);
  } finally {
